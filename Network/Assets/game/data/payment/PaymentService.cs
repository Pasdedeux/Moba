...
namespace common.gameData.payment
{
    public class PaymentCmdCallbackWraper : CmdCallbackWraper
    {
        ...
        public override void DoInWraper(Msg msg)
        {
            ...
            paymentService.OnReqPayOrderReturn(orderid, sdkCode, productId, sdkProductId, productNum,currency, sdkHandler);
       }
    ...
    public class PaymentService
    {
        ...
        public void Load()
        {
            ...
            byte[] bytes = baseDao.Load(CodeMap.Filed.FIELED_PAYMENT_DATA);
            orders = new Orders(bytes);
        ...
        public void OnReqPayOrderReturn(string orderid, int sdkCode, int productId,string sdkProductId, int productNum,string currency, SDKHandler sdkHandler)
        {
            logReport.OnLogReport("reqPayOrderReturn,save oder,order id:"+ orderid);
            sdkHandler.DoPay(orderid, sdkCode, productId, sdkProductId, productNum, OnSDKPayReturn);
        }
        ...
        private void NoticePaySuccess(string orderid, int sdkCode,string recipt,string currency)
        {
            ...
            NetService.getInstance().sendMessage(msg, OnNoticePaySuccessReturn);
        }
        private void OnNoticePaySuccessReturn(Msg msg)
        {
            int rsCode = (int)msg.GetParam(BaseCodeMap.BaseParam.RS_CODE);
            ...
            if (rsCode == BaseCodeMap.BaseRsCode.SUCCESS||rsCode == GameSrvCodeMap.RsCode.ERRO_PAY_FINISHED)
            {
                ...
                if (payReturn != null)
                    payReturn.OnPaySuccess(orderid, productid, updates);
            }
            else 
            {
                ...
                if (payReturn != null)
                    payReturn.NoticeFail(orderid, productid, rsCode);
            }
        }
    ...
    {
        void OnPaySuccess(string orderid, int productid, Dictionary<int, ItemChangeObj> updates);
        void NoticeFail(string orderid, int productid, int rsCode);
    }
}
